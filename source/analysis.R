...
bw_disparities <- incarceration %>%
  ...
  select(
    state, county_name, total_pop_15to64, white_pop_15to64, black_pop_15to64, # select relevant columns
    total_jail_pop, white_jail_pop, black_jail_pop
  ) %>%
  mutate(
    black_pop_percent = (black_pop_15to64 / total_pop_15to64) * 100, # calculate % of black ppl in general pop
  ...
  ) %>% # calculate % of white ppl in jail pop
  filter(
    white_jail_percent <= 100, # I noticed some rows had percentages <100, which indicates some kind of error, so I removed these outliers
    black_jail_percent <= 100
  ) %>%
  select(state, county_name, black_pop_percent, black_jail_percent, white_pop_percent, white_jail_percent) %>%
  mutate(
    black_ratio = black_jail_percent / black_pop_percent,
...
avg_risk_ratio <- incarceration %>%
  ...
  mutate(
    black_risk = black_jail_pop / black_pop_15to64,
...
plot_percentage_data <- function() {
  ...
  plot <- ggplot(data) + # specifying scatterplot parameters
    geom_point(
      mapping = aes(x = percent_of_pop, y = percent_of_jail_pop, color = Race),
      alpha = 0.75
    ) +
    geom_smooth(
      mapping = aes(x = percent_of_pop, y = percent_of_jail_pop, color = Race),
      linetype = "bold", se = FALSE
    ) + # adding linear regression line to emphasize trends
  ...
  plotly_fied <- ggplotly(plot) %>% # making it interactive
    layout(title = list(text = paste0(
      "Comparing the Representation of Racial Groups in Jail vs. the General Population (2018)",
      "<br>",
      "<sup>",
...
#-------------------------------------------------------------------------------
# Map #3: Risk Ratio
get_map_data <- function() { #this fxn creates the data needed to build my map 
  data <- incarceration %>%
    filter(year == 2018) %>% #first, I only want the most recent year
    select( #selecting relevant rows
      fips, state,
      black_jail_pop, black_pop_15to64,
      white_jail_pop, white_pop_15to64
    ) %>%
    ...
    summarise( #calculate summary information 
      black_jail = sum(black_jail_pop, na.rm = T, finite = T),
    ...
    filter(!state %in% c("VT", "HI", "RI", "DE", "CT")) #remove states that lacked required data
  return(data)
}
...
make_map <- function() { #this fxn builds a map using the data created above
  state_rr <- get_map_data() #create df using data wrangling fxn above
  usmap <- plot_usmap(data = state_rr, values = "Relative_Risk") + #create a map of the US w fill representing RR value
    scale_fill_continuous( #choose color for scale and set labels 
  ...
  usmap_ly <- ggplotly(usmap) %>% #make it interactive
    layout(title = list(text = paste0(
      "Comparing the Relative Risk of Incarceration for Black and White Americans (2018)",
      "<br>",
      "<sup>",
